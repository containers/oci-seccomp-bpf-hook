...
@test "Extend existing seccomp profile" {
	...
	showrun $PODMAN_RUN --annotation io.containers.trace-syscall=of:${tmpFile1} ${ALPINE} ls /
	[ "$status" -eq 0 ]
	# sleep two seconds to let the hook finish writing the file
	sleep 2
...
@test "Syscall blocked in input profile remains blocked in output profile" {
	...
	showrun $PODMAN_RUN --annotation io.containers.trace-syscall=of:${tmpFile} ${ALPINE} mkdir /foo
	[ "$status" -eq 0 ]
	# sleep two seconds to let the hook finish writing the file
	sleep 2
	...
	showrun $PODMAN_RUN --security-opt seccomp=${tmpFile} ${ALPINE} mkdir /foo
	[ "$status" -eq 0 ]

	showrun $PODMAN_RUN --annotation io.containers.trace-syscall="if:${BLOCK_MKDIR};of:${tmpFile}" ${ALPINE} mkdir /foo
	[ "$status" -eq 0 ]
	# sleep two seconds to let the hook finish writing the file
	sleep 2
